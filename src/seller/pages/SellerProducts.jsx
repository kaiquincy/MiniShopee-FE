...
import { fetchProducts, deleteProduct } from '../api/seller'
  ...
  return (
    <Box>
      ...
      <Box bg="white" border="1px solid #eee" borderRadius="md">
        <Box display="grid" gridTemplateColumns="80px 1fr 140px 180px" py={2} px={2} borderBottom="1px solid #eee" fontWeight="bold">
          <Box>ID</Box><Box>Tên</Box><Box>Giá</Box><Box>Actions</Box>
        </Box>
        {items.map(p=>(
          <Box key={p.id} display="grid" gridTemplateColumns="80px 1fr 140px 180px" py={3} px={2} borderBottom="1px solid #f6f6f6">
            <Box>#{p.id}</Box>
            <Box>
